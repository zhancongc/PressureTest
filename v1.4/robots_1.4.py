...
class Group(object):
    def __init__(self, robots):
        self.cache = b''
        self.robots = robots
        self.game_response_queue_flag = True
    ...
    def __receive__(self, session, size):
        ...
        if session:
            try:
                ...
                self.logging('[error] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'socket.recv error', e))
                return b''
        else:
            return b''
        return buff
    ...
    def receive(self, session):
        ...
        try:
            ...
            self.logging('[error] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'struct.unpack error', e))
            return None
    ...
    def game_response_gather(self):
        ...
        :return: 不返回数据
        """
        sign = len(self.robots)
        while self.game_response_queue_flag:
            for robot in self.robots:
                robot_response_data = dict()
                res = self.receive(robot.game_session)
                if robot.game_session:
                    if res:
                        robot_response_data.update({
                            'res': res,
                            'robot_index': self.robots.index(robot)
                        })
                        self.game_res_queue.put(robot_response_data)
                        self.logging('[recv] {0} {1}.{2} {3} {4}'.format(robot.userName, self.__class__.__name__, sys._getframe().f_code.co_name, 'game session', res))
                    else:
                        self.logging('[warning] {0} {1}.{2} {3}'.format(robot.userName, self.__class__.__name__, sys._getframe().f_code.co_name, 'receive None'))
                        robot.system_heartbeat(robot.game_session)
                else:
                    print(robot.userName, '没有 game session')
                    sign -= 1
                    if sign == 0:
                        break
        self.game_response_queue_flag = False
    ...
    def match_response_gather(self):
        ...
        while self.match_response_queue_flag:
            sign += 1
            for robot in self.robots:
                # print('这是robot的id', id(robot))
                if robot.match_session:
                    print('这是{0}的match session：{1}'.format(robot.userName, robot.match_session))
                    res = self.receive(robot.match_session)
                    if res:
                        robot_response_data = {
                            'res': res,
                            'robot_index': self.robots.index(robot)
                        }
                        self.match_res_queue.put(robot_response_data)
                        self.logging('[recv] {0} {1}.{2} {3} {4}'.format(robot.userName, self.__class__.__name__, sys._getframe().f_code.co_name, 'match session', res))
                    else:
                        self.logging('[warning] {0} {1}.{2} {3}'.format(robot.userName, self.__class__.__name__, sys._getframe().f_code.co_name, 'receive None'))
                else:
                    print(robot.userName, '没有 match session', robot.is_game_login, sign)
                    if robot.is_game_login and sign % 10 == 0:
                        print(robot.userName, '去找match certificate')
                        robot.player_getRoleInfo2()
            time.sleep(0.1)
    ...
    def game_queue_manager(self):
        ...
        while self.game_response_queue_flag:
            try:
                robot_response_data = self.game_res_queue.get(block=False, timeout=5)
                temp_robot = self.robots[robot_response_data['robot_index']]
                self.logging('[queue] {0} {1}.{2} {3} {4}'.format(temp_robot.userName, self.__class__.__name__, sys._getframe().f_code.co_name, 'handling', robot_response_data['res']))
                func = temp_robot.game_handler_selector(robot_response_data['res'][0])
                print("这是{0}的处理函数:{1}".format(temp_robot.userName, func))
                if func:
                    func(robot_response_data['res'][2])
            except Exception as e:
                self.logging('[warning] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'get None', e))
                time.sleep(1)
    ...
    def match_queue_manager(self):
        ...
        while self.match_response_queue_flag:
            try:
                robot_response_data = self.match_res_queue.get(block=True, timeout=5)
                temp_robot = self.robots[robot_response_data['robot_index']]
                self.logging('[queue] {0} {1}.{2} {3} {4}'.format(temp_robot.userName, self.__class__.__name__, sys._getframe().f_code.co_name, 'handling', robot_response_data['res'][0]))
                func = temp_robot.match_handler_selector(robot_response_data['res'][0])
                if func:
                    func(robot_response_data['res'][2])
            except Exception as e:
                self.logging('[warning] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'get None', e))
    ...
    def logging(self, msg):
        ...
        :return: 不返回数据
        """
        log = "{t} {msg}".format(t=time.strftime("%Y/%m/%d %H:%M:%S", time.localtime()), msg=msg)
    ...
    def run(self):
        """
        :function: 创建一个机器人玩家组，进入游戏
        :return: 不返回数据
        """
        for robot in self.robots:
            robot.user_login(robot.userName, password='123')
            print('I am alive')
        threads = list()
...
class Robots(object):
    ...
    def __connect__(self, server):
        ...
        try:
            ...
            self.logging('[error] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'socket.connect refused', e))
        session.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 9192)
        return session
    ...
    def __send__(self, session, interface, parameters):
        ...
        :return: 不返回数据
        """
        self.logging('[send] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, interface, parameters))
    ...
    def logging(self, msg):
        ...
        """
        pass
        # log = "{t} {userName} {msg}".format(t=time.strftime("%Y/%m/%d %H:%M:%S", time.localtime()), userName=self.userName, msg=msg)
        # with open('log.txt', 'a') as f:
        #     f.write(log+'\n')
        # print(log)
    ...
    def match_login(self):
        ...
        :return: 不返回数据
        """
        self.logging('[state] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'match session exists', self.match_session))
        if self.match_session:
            self.send(self.match_session, "match@login", playerId=self.playerId,  worldId=self.worldId, certificate=self.certificate)
    ...
    def player_getRoleInfo2_handler(self, res):
        ...
        :return: 不返回数据
        """
        taskList = res['data'].get('taskList')
        if isinstance(taskList, list):
            for task in taskList:
                if task['oneTaskInfo'].get('notice'):
                    self.playerTask_ackNotice(task['oneTaskInfo']['notice'])
        if res['data']['gameInfo'].get('state') == 3:
            worldRoom = res['data']['gameInfo'].get('worldRoom')
            if isinstance(worldRoom, dict):
                ...
                print('这是self的id', id(self))
                self.logging('[hint] {0}.{1} {2} '.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'match session setup, game session destroyed'))
                self.match_login()
    ...
    def match_login_handler(self, res):
        ...
        self.logging('[hint] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'game session destroyed', '#'*100))
...
if __name__ == '__main__':
    processes = list()
    robot_groups = group_by(ROBOTS_INDEX_MIN, ROBOTS_INDEX_MAX)
    for elem in robot_groups:
        robots = list()
        for e in elem:
            robot = Robots(ROBOTS_NAME_PREFIX + str(e))
            robots.append(robot)
            print('{0} has connected to the server: {1}'.format(robot.userName, robot.game_server))
        group = Group(robots)
        process = multiprocessing.Process(target=group.run)
