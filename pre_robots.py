...
Description : 创建大量机器人玩家进入game服匹配，进入match服战斗 原版
"""
...
try:
    ...
    SERVER['port'] = conf.get('server', 'port')
    JOIN_INTERVAL = conf.getfloat('robots', 'join_interval')
    ROBOTS_NAME_PREFIX = conf.get('robots', 'robots_name_prefix')
    ROBOTS_INDEX_MIN = conf.get('robots', 'robots_index_min')
    ROBOTS_INDEX_MAX = conf.get('robots', 'robots_index_max')
    ...
    def game_response_gather(self):
        ...
        while self.game_session:
            ...
            if res:
                ...
                self.logging('[recv] {0}.{1} {2}'.format(self.__class__.__name__,\
    ...
    def game_queue_manager(self):
        ...
        while self.game_session:
            ...
            try:
                ...
                self.logging('[error] {0}.{1} {2} {3}'.format(self.__class__.__name__,\
