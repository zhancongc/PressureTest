...
class Group(object):
            ...
            for robot in self.robots:
                ...
                if robot.game_session:
                    if res:
                        ...
                        self.logging('[recv] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'game session', res))
                    else:
                        self.logging('[warning] {0}.{1} {2}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'receive None'))
            ...
            for robot in self.robots:
                ...
                if robot.match_session:
                    ...
                    if res:
                        ...
                        self.logging('[recv] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'match session', res))
                    else:
                        self.logging('[warning] {0}.{1} {2}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'receive None'))
                else:
                    print(robot.userName, '没有 match session')
                    time.sleep(1)
    ...
    def game_queue_manager(self):
        ...
        while self.game_response_queue_flag:
            try:
                robot_response_data = self.game_res_queue.get(block=True, timeout=5)
                self.logging('[queue] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'handling', robot_response_data['res'][0]))
                func = robots[robot_response_data['robot_index']].game_handler_selector(robot_response_data['res'][0])
                if func:
    ...
    def match_queue_manager(self):
        ...
        while self.match_response_queue_flag:
            try:
                ...
                self.logging('[queue] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'handling', robot_response_data['res'][0]))
                func = robots[robot_response_data['robot_index']].match_handler_selector(robot_response_data['res'][0])
                if func:
                    func(robot_response_data['res'][2])
            except Exception as e:
                self.logging('[warning] {0}.{1} {2} {3}'.format(self.__class__.__name__, sys._getframe().f_code.co_name, 'get None', e))
